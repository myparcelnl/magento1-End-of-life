 ...
 */
class TIG_MyParcelBE_MyparcelAdminhtml_ShipmentController extends Mage_Adminhtml_Controller_Action
{
    /**
     * Used module name in current adminhtml controller.
     */
    protected $_usedModuleName = 'TIG_MyParcelBE';
    ...
    public function massCreateShipmentsAction()
    {
        ...
        try {
            ...
            foreach ($orderIds as $orderId) {
                ...
                } catch (TIG_MyParcelBE_Exception $e) {
    ...
    public function createConsignmentAction()
    {
        ...
        if (!empty($selectedConsignmentOptions['shipment_type'])) {
            ...
            try {
                ...
                if($helper->getPgAddress($shipment->getOrder()) && ($shipmentType != TIG_MyParcelBE_Model_Shipment::TYPE_NORMAL && $shipmentType != 'default')){
            ...
            if ($shipmentType != TIG_MyParcelBE_Model_Shipment::TYPE_NORMAL) {
    ...
    public function massPrintLabelsAction()
    {
        ...
        foreach ($shipments as $shipment) {
            try {
                if (!$shipment->hasConsignmentId()) {
                    ...
                    if($helper->getPgAddress($shipment->getOrder()) && $type != TIG_MyParcelBE_Model_Shipment::TYPE_NORMAL && $type != 'default'){
                        $shipment_url = Mage::helper('adminhtml')->getUrl('adminhtml/sales_order_shipment/view',array('shipment_id' => $shipment->getShipment()->getId()));
                        throw new TIG_MyParcelBE_Exception(
                            $helper->__('The selected shipment type cannot be used. Pakjegemak shipments can only be created with the normal shipment type.<br/> The Magento shipment has been created without a MyParcel shipment, select a different shipment type or go to the shipment page to create a single MyParcel shipment. <a target="_blank" href="%s">View shipment</a>',$shipment_url),
                            'MYPA-0023'
        ...
        /**
         * @var $api TIG_MyParcelBE_Model_Api_MyParcel
         */
     ...
     *                                         Mage_Sales_Model_Shipment or TIG_MyParcelBE_Model_Shipment objects.
    ...
    protected function _loadAndCheckShipments($shipmentIds, $loadMyParcelShipments = false, $throwException = true, $keyIsConsignmentId = true)
    {
        ...
        foreach ($shipmentIds as $shipmentId) {
             ...
             * Load the shipment.
             *
             * @var Mage_Sales_Model_Order_Shipment|TIG_MyParcelBE_Model_Shipment|boolean $shipment
             */
     ...
     *
     * @return boolean|Mage_Sales_Model_Order_Shipment|TIG_MyParcelBE_Model_Shipment
     */
     ...
     * @param int $shipmentId
     *
     * @return TIG_MyParcelBE_Model_Shipment
     */
     ...
     *
     * @return int|TIG_MyParcelBE_Exception
     *
     *
     * @throws TIG_MyParcelBE_Exception
     */
